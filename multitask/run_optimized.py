...
from my_blue_classifier_optimized import AutoTokenizer,loadmodel,loadData,get_features,MyDataset,train,evaluation,InputFeatures,relativePath,load_tokenizer
...
def main():
    ...
    if len(models_in_file) == 0 and do_train:
        ...
        """
        model = loadmodel(module_name=module_name, frozen=False, task_num=task_num)  # load default model
        ...
        # # start training
        trainer = train(base_model=model, data_Sets=[mednli_train_data],
                        device='cuda',batch_size=6, shuffle=True,task_num= task_num, accumulate_iter=3
                        )
        # trainer = train(base_model=model,
        #                 data_Sets=[biosses_train_data, chemprot_train_data, mednli_train_data, hoc_train_data,
        #                            ddi2010_train_data], batch_size=4,
        ...
        trainer = train(base_model=base_model, data_Sets=[],
                        device='cuda',batch_size=6, shuffle=True, task_num=1,accumulate_iter=3)
        trainer.run(train_epoch)
    ...
    if do_eval and len(models_in_file) > 0:
        ...
        for model in models_in_file:
            ...
            # evaluation(model=initial_model,model_dict_dir=recent_model_dict, eval_dataSets=[mednli_test_data],device=device, batch_size=20)
